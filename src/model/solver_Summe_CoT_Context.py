...
# =========================
# Prompt builders (summe_adapted)
# =========================

def _summe_rubric_block():

    return (
        "You are a segment evaluator for the SumMe dataset.\n"
        ...
        "1) Title/Thematic Relevance (0–35)\n"
        "   - Directly delivers the title’s core subject: aircraft motion, bus navigating tunnel, organized play, chef cooking, fire transformation, jump/slide, machinery teamwork, wildlife in natural habitat, coral close-ups, iconic architecture in scenic context, kids playing with leaves, tranquil beach.\n"
        "   - Low if off-focus (e.g., urbanized wildlife, crowded beach, generic close-ups without context).\n"
        "2) Action/Interaction & Skill (0–20)\n"
        "   - Visible motion/operation/behavior/transformation (aircraft landing, bus maneuver, cycling/paintball play, chef handling food, object burning, jump/slide sequence, excavator teamwork).\n"
        "   - Static establishing shots low unless essential immediate setup.\n"
        "3) Detail & Visibility (0–15)\n"
        "   - Informative clarity: cockpit instruments, coral texture, iron lattice, carvings, chef’s hands/tools, machinery.\n"
        "   - Penalize blur/dark/occlusion.\n"
        "4) Informational Uniqueness (0–15)\n"
        "   - New step/angle/outcome vs. adjacent scenes; penalize near-duplicates (runway/fence/rock/grass repeats).\n"
        "5) Narrative Progression (0–15)\n"
        "   - Bridges setup→action→result or shows a turning point/transformation (e.g., anticipation before jump, ignition→burn, approach→touchdown, entry into tunnel, teamwork at climax).\n\n"
        "### Category checklists (quick boosters)\n"
        "- AVIATION (Air_Force_One / Cockpit_Landing / St Maarten Landing): plane movement, cockpit clarity, runway alignment, vibrant ocean/crowd; weak if static runway/fence/blur sky.\n"
        "- NATURE/ADVENTURE (Base jumping / Jumps / Playing_on_water_slide): prep→execution→landing, equipment handling; weak if only generic scenery or idle crowd.\n"
        "- WILDLIFE/SERENITY (Bearpark_climbing / Saving dolphines): natural behavior, tranquil seascape; weak if urban setting or crowded beach.\n"
        "- ORGANIZED SPORTS (Bike Polo / Paintball): marked field, barriers, targets, gear, structured play; weak if casual idling or grass-only close-up.\n"
        "- INDUSTRIAL/LOGISTICS (Bus_in_Rock_Tunnel / Excavators river crossing): vehicle/machinery operating in challenge context, teamwork visible; weak if only rock/bucket close-up.\n"
        "- EVENT/UNIQUE (Car_railcrossing): theatrical uniqueness (blue-skinned participants, mechanical claw); weak if generic accident aftermath.\n"
        "- COOKING (Cooking): chef active on teppan, hygiene cues, concrete step; weak if static ingredients only.\n"
        "- ARCHITECTURE (Eiffel Tower / Notre_Dame / Statue of Liberty): craftsmanship+context (sunrise/sunset, carvings, lively boardwalk); weak if isolated close-up or blurry skyline.\n"
        "- UNDERWATER (Scuba): coral close-ups with texture/color; weak if wide undetailed reef.\n"
        "- KIDS/PLAY (Kids_playing_in_leaves): children’s energetic play with leaves; weak if driveway/house/cars dominate.\n"
        "- FIRE/TRANSFORMATION (Fire Domino): dynamic burning process; weak if only initial intact object.\n\n"
        "### Standard penalties (apply within dimensions)\n"
        "- OFF_TOPIC: −10\n"
        "- STATIC_GENERIC (idle, no task/motion): −8\n"
        "- LOW_VISIBILITY (dark/blur/occluded): −6\n"
        "- REDUNDANT_REPEAT (near-duplicate): −6\n"
        "Note: For serenity titles, STATIC_GENERIC only if static fails to convey serenity.\n\n"
        "### Calibration ladder (combat score inflation)\n"
        "- 90–100: indispensable climax/core step with clear action/transform detail, unique, advances outcome.\n"
        "- 75–89: strong action/operation/behavior or craftsmanship+context; minor gaps.\n"
        "- 60–74: supportive context or partial step with clarity.\n"
        "- 40–59: decorative/light context; weak linkage to intent.\n"
        "- 0–39: off-topic/static/poor visibility/redundant.\n\n"
        "### Mandatory internal calculation (do not reveal)\n"
        "Compute sub-scores: R=Relevance, A=Action, D=Detail, U=Uniqueness, N=Narrative.\n"
        "Apply penalties within each dimension. If target ≈ prev/next, set U ≤ 5.\n"
        "FINAL = round(0.35*R + 0.20*A + 0.15*D + 0.15*U + 0.15*N − Penalties); clamp [0,100].\n"
        "Use the full scale; typical scenes 45–60 unless clearly strong/weak.\n"
...
def generate_prompt_target_only(video_title: str,
                                video_description: str,
                                target_scene_text: str,
                                user_query: str) -> str:
    """
    Prompt for the first or last scene (no local context).
    Encourages JSON output; integer-only fallback allowed by parser.
    """
    header = _summe_rubric_block()
    uq = f"\n- user_preference (optional): {user_query.strip()}" if user_query.strip() else ""
    vt = f"{video_title}".strip()
    prompt = (
        f"{header}\n"
        f"## Filled Inputs\n"
        f"- video_title: {vt}\n"
        f"- global_brief: {video_description.strip()}\n"
        f"- segment_desc: {target_scene_text.strip()}\n"
        f"{uq}\n"
        f"Return ONLY the JSON above (or a single integer 0–100 if JSON is impossible)."
    )
    return prompt


def generate_prompt_with_context(video_title: str,
                                video_description: str,
                                 prev_scene_text: str,
                                 target_scene_text: str,
                                 next_scene_text: str,
...
class SceneScorer:
    ...
    def parse_single_score(self, output_text: str) -> int:
        ...
        try:
            ...
            print(f"⚠️ Error parsing score from: {output_text[:120]}... ({e}) -> use 50")
            return 50
    ...
    def compute_scenes_score(self, description_file_name: str, user_query: str, video_title: str = ""):
        """
        新版核心评分函数：
        - 支持 JSON 或 整数输出；优先 JSON
        - 首尾场景：无上下文；中间场景：带 prev/next（仅用于唯一性/推进）
        - video_title 优先使用 scene_desc 中的字段，否则回退为外部传入值
        """
        try:
        ...
        print(f"🔍 Found {num_scenes} scenes for scoring (SumMe-adapted rubric)")
        video_desc = descriptions.get('video_description', '').strip()

        # video_title 优先从 JSON 取
        json_title = descriptions.get('video_title', '').strip()
        if json_title:
            vt = json_title
        else:
...
def solve(args):
    ...
    for video_name in videos:
        ...
        # 5. 计算评分（SumMe-adapted rubric）
        try:
            print("🧮 Computing scene scores (SumMe-adapted rubric)...")
            # 尝试从描述文件中读取 video_title；若无，回退为 video_name
            try:
                with open(scene_desc_file, "r") as jf:
                    _d = json.load(jf)
                json_title = _d.get("video_title", "").strip()
            except Exception:
                json_title = ""
            vt_for_prompt = json_title if json_title else video_name

            scene_scores = scorer.compute_scenes_score(
                scene_desc_file, args.user_query, video_title=vt_for_prompt
            )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Video Scene Scoring Pipeline (SumMe-adapted rubric, GPT-5)")
    ...
    parser.add_argument("--work_dir", type=str, required=True,
                        help="Working directory for output (e.g., /usr1/home/.../summe_metadata_gpt)")
