...
def _tvsum_rubric_block():
    """Core rubric text for tvsum_v2 (score_only)."""
    return (
        "You are a segment evaluator for the TVSum dataset.\n"
        "Score ONLY the target scene using the global video description.\n\n"
        ...
        "Apply penalties within each dimension.\n"
...
def generate_prompt_target_only(video_description: str,
                                ...
                                user_query: str) -> str:
    """
    Prompt for ANY scene (no local context).
...
# （保留占位：如果你后续想再启用上下文，这个函数仍可存在但此版本不使用）
def generate_prompt_with_context(video_description: str,
                                 prev_scene_text: str,
                                 target_scene_text: str,
                                 next_scene_text: str,
                                 user_query: str) -> str:
    # 不使用上下文版本：为了与原文件结构完全一致，保留函数定义但不调用
    header = _tvsum_rubric_block()
    q = ""
    if user_query.strip():
        q = (
    ...
    prompt = (
        f"{header}\n\n"
        f"### Inputs (context is ignored in this no-context setting)\n"
        f"- Global video description:\n{video_description.strip()}\n\n"
        f"- TARGET scene (score this one ONLY):\n{target_scene_text.strip()}\n"
        f"{q}\n\n"
        f"Return one integer."
    )
    return prompt
...
class SceneScorer:
    ...
    def compute_scenes_score(self, description_file_name: str, user_query: str):
        ...
        print(f"🔍 Found {num_scenes} scenes for scoring (tvsum_v2, target-only NO CONTEXT)")
...
def solve(args):
    ...
    for video_name in videos:
        ...
        # 5. 计算评分（tvsum_v2 target-only，NO CONTEXT）
        try:
            print("🧮 Computing scene scores (tvsum_v2, no-context)...")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Video Scene Scoring Pipeline (tvsum_v2 target-only, NO CONTEXT, GPT-5)")
