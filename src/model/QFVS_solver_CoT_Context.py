...
def _build_prompt_multiq_with_ctx(video_description: str,
    ...
    中间场景的“带上下文”版本：
    —— 仍旧只对“目标场景”打分；
    —— 明确规定前后文仅用于判定“是否新信息/是否重复”的±5 小幅调整；
    —— 输出依旧是 K 个逗号分隔的整数。
    """
    header = _qfvs_rubric_block()
    ctx_rule = (
        "### Context handling (read carefully; do this INTERNALLY before scoring)\n"
        "- Briefly internalize the Previous and Next scenes separately (do NOT output notes). "
        "Keep only key facts: who, what key action, stage (setup/key/aftermath), and whether the target is NEW or DUPLICATED vs both neighbors.\n"
        "- Scoring priority: Derive the BASE score PRIMARILY from the TARGET scene description and the Global video description, strictly following the rubric.\n"
        "- Small context adjustment (±5 applied to the final score):\n"
        "  + Add up to +5 ONLY if the TARGET clearly introduces NEW information/stage vs BOTH neighbors.\n"
        "  – Subtract up to -5 ONLY if the TARGET is largely DUPLICATED/redundant vs BOTH neighbors.\n"
        "  0 If evidence is unclear or mixed, apply NO adjustment (0). This adjustment is small and conservative.\n"
        "- Always SCORE ONLY THE TARGET; prev/next are reference for the small ±5 adjustment, not direct scoring evidence.\n"
    )
    qsec = ""
    if user_queries:
        qsec += "User Queries (evaluate the scene AGAINST EACH query, and output one score per query in order):\n"
...
def _call_openai(prompt: str, model: str, timeout: int = 60, max_retries: int = 5) -> str:
    """
    带超时与退避重试的 ChatCompletion 调用。兼容老 SDK；gpt-5 不传 temperature。
    """
    backoff = [2, 4, 8, 15, 30]
...
def run(args):
    try:
        ...
        # ---------- 核心评分循环（多查询一行 + 中间段用上下文） ----------
        num_queries = len(user_queries_map)
        query_scores = [[] for _ in range(num_queries)]  # 每个查询一个分数序列
        ...
        for idx, scene_id in enumerate(scene_indices, 1):
            ...
            # 仅当“不是第一段也不是最后一段”时使用上下文 prompt
            if scene_id > 1 and scene_id < scene_indices[-1]:
                prev_desc = descriptions.get(f'scene_{scene_id-1}_description', '')
                next_desc = descriptions.get(f'scene_{scene_id+1}_description', '')
                prompt    = _build_prompt_multiq_with_ctx(video_desc, prev_desc, part_desc, next_desc, uq_list)
            else:
                prompt    = _build_prompt_multiq(video_desc, part_desc, uq_list)

            time.sleep(0.05)  # 轻限速，避免过快触发限流
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MAIN (QFVS: detect existing files; multi-query per scene scoring with latest prompt; with optional context for middle scenes)")
